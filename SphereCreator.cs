...
namespace GK_Lab_4_v2
{
    public class SphereCreator
    {
        ...
        private int addVertex(Point3D p)
        {
            ...
            vertices.Add(new Vector4((float)(p.X / length), (float)(p.Y / length), (float)(p.Z / length), 1));
            return index++;
        }
        private void addTriangle(List<Triangle> tris, int v1, int v2, int v3)
        {
            tris.Add(new Triangle(vertices[v1], vertices[v2], vertices[v3], Color.Red));
        }
        ...
        public List<Triangle> Create(int recursionLevel)
        {
            ...
            for (int i = 0; i < recursionLevel; i++)
            {
                ...
                foreach (var tri in faces)
                {
                    ...
                    int[] ind = new int[3];
                    ind[0] = vertices.IndexOf(tri.points[0]);
                    ind[1] = vertices.IndexOf(tri.points[1]);
                    ind[2] = vertices.IndexOf(tri.points[2]);

                    int a = getMiddlePoint(ind[0], ind[1]);
                    int b = getMiddlePoint(ind[1], ind[2]);
                    int c = getMiddlePoint(ind[2], ind[0]);

                    addTriangle(faces2, ind[0], a, c);
                    addTriangle(faces2, ind[1], b, a);
                    addTriangle(faces2, ind[2], c, b);
                    addTriangle(faces2, a, b, c);
                }
                faces = faces2;
            }
            return faces;
